...
var markerDraw = (function (config, xScale, markerState) {
    return function (selection) {
        var marker = selection.selectAll('.markerGrp').data([markerState]);

        var markerGrp = marker.enter().append('g').classed('markerGrp', true).attr('transform', function (d) {
            return 'translate(' + xScale(d.date) + ')';
        });

        markerGrp.append('rect').classed('marker', true).attr('x', -1).attr('y', -20).attr('height', '20').attr('width', '3');

        selection.selectAll('.markerGrp').attr('transform', function (d) {
            return 'translate(' + xScale(d.date) + ')';
        });
    };
});
...
var axis = (function (d3, config, xScale, breakpointLabel) {
    var labelWidth = config.label.width,
        _config$axis = config.axis,
        formats = _config$axis.formats,
        verticalGrid = _config$axis.verticalGrid,
        tickPadding = _config$axis.tickPadding,
        locale = config.locale,
        numberDisplayedTicks = config.numberDisplayedTicks,
    ...
    return function (selection) {
        ...
        }).ticks(numberDisplayedTicks[breakpointLabel]).tickPadding(tickPadding);

        if (verticalGrid) axisTop.tickSizeInner(-(selection.data()[0].length * lineHeight));
...
var getBreakpointLabel = function getBreakpointLabel(breakpoints, point) {
...
var bounds = (function (config, xScale) {
    return function (selection) {
        ...
        var boundTextGroup = bounds.enter().filter(function (_, i) {
            return !i;
        }).append('g').classed('bound', true).attr('transform', 'translate(' + labelWidth + ', ' + (lineHeight * numberRows + margin.top - 9) + ')');

        boundTextGroup.append('text').classed('start', true).text(dateFormat(xScale.domain()[0]));

        boundTextGroup.append('text').classed('end', true).attr('x', xScale.range()[1] - margin.right).attr('text-anchor', 'end').text(dateFormat(xScale.domain()[1]));
...
var dropLine = (function (config, xScale, d3) {
    return function (selection) {
        ...
        var drops = g.append('g').classed('drops', true).attr('transform', function () {
            ...
            if (idx == 0) {
                d3.select(nodes[idx]).selectAll('.drop').data(function (d) {
                    return d.data;
                }).enter().append('circle').classed('drop', true).on('click', onClick).on('mouseover', onMouseOver).on('mouseout', onMouseOut).attr('r', 5).attr('fill', function (d) {
                    var memo = d.memo;
                ...
                }).attr('y', '-9').attr('x', function (d) {
                    return xScale(d.date);
                }).attr('width', function (d) {
                    return xScale(new Date(d.date.getTime() + d.msec)) - xScale(d.date);
                });
            }
        });
        ...
        selection.selectAll('.drop').attr('cx', function (d) {
            return xScale(d.date);
        });
        selection.selectAll('.clip').attr('x', function (d) {
            return xScale(d.date);
        }).attr('width', function (d) {
            return xScale(new Date(d.date.getTime() + d.msec)) - xScale(d.date);
        });
...
var zoom = (function (d3, svg, config, xScale, draw) {
    var _config$label = config.label,
    ...
    zoom.on('zoom', function (event, args) {
        var transform = getShiftedTransform(event.transform, labelsWidth, labelsPadding, d3);
...
var index = (function (_ref) {
    ...
    var initChart = function initChart(selection) {
        ...
        var config = index$1(customConfiguration || {}, defaultConfiguration(d3));
        ...
        svg.merge(root).attr('height', function (d) {
            return (d.length + 1) * lineHeight + margin.top + margin.bottom + 10;
        });

        var zoomContainer = svg.append('g').classed('viewport', true).attr('height', '30').attr('width', width).attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');

        zoomContainer.append('rect').attr('height', '30').attr('width', width).attr('opacity', '0').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');

        var stampContainer = svg.append('g').classed('timeline-pf-timestamp', true).attr('height', 30).attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
    ...
    var chart = function chart(selection) {
        chart._initialize = function () {
            return initChart(selection);
        };
        chart._initialize();
    ...
    var draw = function draw(config, scale) {
        return function (selection) {
            ...
            //marker.updateMarker();
            selection.data(selection.data()).call(axis(d3, config, scale, chart.currentBreakpointLabel)).call(dropLine(config, scale, d3)).call(bounds(config, scale)).call(markerDraw(config, scale, markerState));
        };
    };
